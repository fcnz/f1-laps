<script lang="ts">
	...
	const buildChartOptions = (data: PageProps['data']) => {
		...
		const gapToLaderSeries = data.drivers.map((driver) => {
			const seriesData: [number, number][] = [];
			driver.intervals.forEach((interval) => {
				seriesData.push([
					deltaMinutes(new Date(interval.date), new Date(data.session.date_start)),

					// deltaMinutes(new Date(interval.date), new Date(data.session.date_start)) / 10 -
